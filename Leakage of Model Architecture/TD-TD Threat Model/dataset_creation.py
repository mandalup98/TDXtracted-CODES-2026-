...
for bench in benchmarks:
    ...
    if os.path.exists(file_path):
        try:
            ...
            values = df[1:].astype(float).astype(int).values.flatten()
...
df_dataset.to_csv("TD_DNN_memory_activity.stalls_l1d_miss.csv", index=False)  # Optional: CSV for readability
