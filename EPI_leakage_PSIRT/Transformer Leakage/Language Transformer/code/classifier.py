...
class BetterCNN1D(nn.Module):
    """
    A stronger but still simple 1D CNN:
...
def compute_norm_stats(X_train: np.ndarray):
    # X_train: (N,2,100). compute per-channel mean/std over N and time axis.
    mu = X_train.mean(axis=(0, 2), keepdims=True)  # (1,2,1)
...
def class_weights(y_train: np.ndarray, num_classes: int) -> np.ndarray:
    ...
    w = counts.sum() / (counts + 1e-9)  # inverse frequency
    w = w / w.mean()
...
def save_confmat_out_of_100(cm_counts: np.ndarray, class_names, out_pdf: str):
    total = cm_counts.sum()
    cm_pct = (cm_counts / total * 100.0) if total > 0 else cm_counts.astype(float)

    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111)
    im = ax.imshow(cm_pct, interpolation="nearest")
    ax.set_title("Confusion Matrix (percent of all test samples, sums to 100)")
...
def save_confmat_percent_overall(cm_counts: np.ndarray, class_names, out_pdf: str):
    total = cm_counts.sum()
    cm_pct = (cm_counts / total * 100.0) if total > 0 else cm_counts.astype(float)
    ...
    ax = fig.add_subplot(111)
    im = ax.imshow(cm_pct, interpolation="nearest")
    ax.set_title("Confusion Matrix (overall %, sums to 100%)")
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
...
def save_confmat_percent_row(cm_counts: np.ndarray, class_names, out_pdf: str):
...
def main():
    ...
    # class weighted loss to avoid collapse
    ...
    out_pdf = os.path.join(args.out_dir, "confusion_matrix_out_of_100.pdf")
    save_confmat_out_of_100(cm_counts, class_names, out_pdf)
    print(f"[OK] Saved confusion matrix (percent out of 100): {out_pdf}")
    print(f"[OK] Saved best model: {best_path}")

    out_overall = os.path.join(args.out_dir, "confmat_overall_100pct.pdf")
    out_row = os.path.join(args.out_dir, "confmat_row_100pct.pdf")

    save_confmat_percent_overall(cm_counts, class_names, out_overall)
    save_confmat_percent_row(cm_counts, class_names, out_row)

    print(f"[OK] Saved overall-% confusion matrix: {out_overall}")
    print(f"[OK] Saved row-% confusion matrix: {out_row}")
